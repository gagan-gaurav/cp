...
void rsq(){                    // O(N*logN)
	...
	for(int j = 1; j <= K; j++){
		for(int i = 0; i + (1 << j) <= a.size(); i++)
			st[i][j] = st[i][j - 1] + st[i + (1 << (j - 1))][j - 1]; //  st[i][j] = f(st[i][j - 1], st[i + (1 << (j - 1))][j - 1])
	}
}
...
void rmq(){                       // O(N*logN)
	...
	for(int j = 1; j <= K; j++){
		for(int i = 0; i + (1 << j) <= a.size(); i++)
			st[i][j] = min(st[i][j - 1], st[i + (1 << (j - 1))][j - 1]);  //  st[i][j] = f(st[i][j - 1], st[i + (1 << (j - 1))][j - 1])
	}
}
...
void rgq(){							// O(N*logN)
	...
	for(int i = 0; i < (int)a.size(); i++) st[i][0] = a[i];
	for(int j = 1; j <= K; j++){
		for(int i = 0; i + (1 << j) <= a.size(); i++){
